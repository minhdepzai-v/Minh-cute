...
getgenv().Team = "Marines"
    if lp.Team ~= nil then return end
...
JoinTeam()
function FindEnemiesInRange(targetList, enemyList)
local Players = game:GetService("Players")
local LocalPlayer = Players.LocalPlayer
    ...
    for _, enemy in ipairs(enemyList) do
        ...
        if not enemy:GetAttribute("IsBoat")
        and humanoid
        and humanoid.Health > 0
        and head
        and (playerPos - head.Position).Magnitude <= 60
        and enemy ~= character then
            table.insert(targetList, { enemy, head })
            lastHead = head
        end
    end
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and player.Character then
            ...
            if head and (playerPos - head.Position).Magnitude <= 60 then
                table.insert(targetList, { player.Character, head })
                lastHead = head
            end
        end
    end
...
function AttackNoCoolDown()
    ...
    local targetHead = FindEnemiesInRange(targets, enemies)
    ...
    pcall(function()
        local ReplicatedStorage = game:GetService("ReplicatedStorage")
        local RegisterAttack = ReplicatedStorage
            :WaitForChild("Modules")
            :WaitForChild("Net")
            :WaitForChild("RE")
            :WaitForChild("RegisterAttack")
        local RegisterHit = ReplicatedStorage
            :WaitForChild("Modules")
            :WaitForChild("Net")
            :WaitForChild("RE")
            :WaitForChild("RegisterHit")
        if #targets > 0 then
...
local Window = Main:CreateWindow({
    Title = "MTX Hub",
    SubTitle = "Blox fruit | by @minh20344",
    TabWidth = 160,
...
Status = Window:AddTab({ 
        Title = "Trạng Thái Máy Chủ", Icon = "server" 
}),
Settings = Window:AddTab({ 
        Title = "Cài Đặt", Icon = "settings" 
}),
Main = Window:AddTab({ 
        Title = "Chung", Icon = "home" 
}),
Plr = Window:AddTab({ 
        Title = "Người Chơi", Icon = "user" 
}),
Shop = Window:AddTab({ 
        Title = "Cửa Hàng", Icon = "shopping-cart" 
}),
Quest = Window:AddTab({ 
        Title = "Nhiệm Vụ", Icon = "scroll" 
}),
Sub = Window:AddTab({ 
        Title = "Vật Phẩm", Icon = "swords" 
}),
Sea = Window:AddTab({ 
        Title = "Sự Kiện Biển", Icon = "anchor" 
}),
Tween = Window:AddTab({ 
        Title = "Dịch Chuyển", Icon = "locate" 
}),
Race = Window:AddTab({ 
        Title = "Nâng Tộc", Icon = "user-plus" 
}),
Fruit = Window:AddTab({ 
        Title = "Trái", Icon = "apple" 
}),
Raid = Window:AddTab({ 
        Title = "Tập Kích", Icon = "" 
}),
Misc = Window:AddTab({ 
        Title = "Khác", Icon = "circle-ellipsis" 
})
}
...
ImageButton.Image = "http://www.roblox.com/asset/?id=99393273304767"
...
--Function--
function CheckLv()
    local a = game.Players.lp.Data.Level.Value;
    if Sea1 then
        if a == 1 or a <= 9 then
...
local DungeonCFrames = {
    CFrame.new(-227.457, 236.539, -426.895),
    CFrame.new(-356.440, 236.539, -446.551),
    CFrame.new(-489.517, 236.539, -430.249),
}
local CitizenTable = {
    "Forest Pirate",
    "Captain Elephant"
...
local function ClearESP()
    for _, v in pairs(IslandESP) do
        if v.Gui then v.Gui:Destroy() end
    end
    table.clear(IslandESP)
end

-- 🏝️ Lấy part đại diện island (KHÔNG tạo part mới)
local function GetIslandPart(model)
    if model.PrimaryPart then
        return model.PrimaryPart
    end

    -- lấy part ở gần tâm nhất
    ...
    for _, v in pairs(model:GetDescendants()) do
        if v:IsA("BasePart") then
            local d = (v.Position - pivot).Magnitude
            if d < bestDist then
                bestDist = d
                bestPart = v
            end
...
-- 🏝️ Check island THẬT
local function IsRealIsland(model)
    if not model:IsA("Model") then return false end
    if model:GetAttribute("IsIsland") == false then return false end
    ...
    for _, v in pairs(model:GetDescendants()) do
        if v:IsA("BasePart") then
            partCount += 1
            sizeSum += v.Size.Magnitude
            if partCount >= 25 and sizeSum >= 300 then
                return true
            end
        end
    end
...
-- 🏝️ Tên island
local function GetIslandName(model)
    local sv = model:FindFirstChild("Name", true)
...
local function CreateESP(model)
    local part = GetIslandPart(model)
    ...
    table.insert(IslandESP, {
        Model = model,
        Part = part,
        Label = txt,
        Gui = gui,
        Name = GetIslandName(model)
    })
end

-- ✅ ENABLE
function EnableIslandESP()
    ClearESP()
    ESPEnabled = true

    local count = 0
    for _, model in pairs(workspace:GetDescendants()) do
        if IsRealIsland(model) then
            CreateESP(model)
            count += 1
            if count >= 30 then break end -- 🔥 giới hạn island
        end
    end
end

-- ❌ DISABLE
function DisableIslandESP()
    ESPEnabled = false
    ClearESP()
end
...
-- 📦 Tạo ESP
local function CreateESP(part, index)
    ...
    billboard.Parent = workspace -- 🔥 QUAN TRỌNG
...
function EnableChestESP()
    ...
    for _, obj in pairs(workspace:GetDescendants()) do
        ...
        -- Chest là MODEL
        elseif obj:IsA("Model") and string.find(obj.Name:lower(), "chest") then
            local part = obj.PrimaryPart or obj:FindFirstChildWhichIsA("BasePart")
            if part then
                count += 1
                CreateESP(part, count)
            end
        end
    end
end
...
RunService.RenderStepped:Connect(function()
    ...
    for plr, esp in pairs(ESP) do
        if esp.HRP and esp.Humanoid and esp.HRP.Parent then
            local dist = (myHRP.Position - esp.HRP.Position).Magnitude
...
-- Xóa ESP
local function ClearESP()
    for _, v in pairs(FruitESP) do
        if v.Gui then
            v.Gui:Destroy()
        end
    end
    table.clear(FruitESP)
end

-- Check đã esp chưa
local function IsEsped(obj)
    for _, v in pairs(FruitESP) do
        if v.Obj == obj then
            return true
        end
...
local function CreateESP(obj)
    if IsEsped(obj) then return end
...
-- ✅ Bật ESP Fruit (chỉ quét model hợp lý)
function EnableFruitESP()
    ClearESP()
    ESPEnabled = true

    for _, obj in pairs(workspace:GetChildren()) do
        -- Fruit spawn thường là Model
...
RunService.RenderStepped:Connect(function()
    ...
    for _, esp in pairs(FruitESP) do
        if esp.Part and esp.Part.Parent then
            ...
            local fruitName =
                esp.Obj:GetAttribute("FruitName")
                or (esp.Obj:FindFirstChild("FruitName") and esp.Obj.FruitName.Value)
                or esp.Obj.Name
            ...
            if GearESP then
                if part.Name == "MeshPart" then
                    if not part:FindFirstChild("NameEsp") then
                        ...
                        text.TextColor3 = Color3.fromRGB(80, 245, 245)
                    end

                    if part:FindFirstChild("NameEsp") and lp.Character and lp.Character:FindFirstChild("Head") then
                        local distance = (lp.Character.Head.Position - part.Position).Magnitude / 3
                        part.NameEsp.TextLabel.Text =
                            part.Name .. "\n" .. v20(distance) .. " M"
                    end
                end
            else
    ...
    for _, tool in ipairs(Backpack:GetChildren()) do
        ...
        if eatRemote then
            replicated.Remotes.CommF_:InvokeServer(
                ...
                Backpack:FindFirstChild(tool.Name)
            )
        end
    end
end
CollectFruitEnabled = false
function CollectFruit(enabled)
    CollectFruitEnabled = enabled
    ...
    task.spawn(function()
        while CollectFruitEnabled do
            ...
            for _, obj in ipairs(workspace:GetDescendants()) do
                if not CollectFruitEnabled then break end
                if obj.Name == "Fruit" and obj:IsA("Model") then
                    Fly(obj, 300)
                    task.wait(1)
                end
            end
...
function GetWeaponInventory(weaponName)
    local inventory = game:GetService("ReplicatedStorage")
        .Remotes.CommF_:InvokeServer("getInventory")
    for _, item in pairs(inventory) do
        if type(item) == "table" then
            if item.Type == "Sword" and item.Name == weaponName then
                return true
            end
        end
    end
...
function CheckMaterial(materialName)
    ...
    for _, item in pairs(inventory) do
        if type(item) == "table" then
            if item.Type == "Material" and item.Name == materialName then
                return item.Count
            end
        end
    end
...
local function canAct()
    ActionCount += 1
    if ActionCount > Config.ACTION_LIMIT_PER_MIN then
...
local SafeExecutor = {}
function SafeExecutor:Execute(func, ...)
...
function SafeExecutor:SafeMove(cf)
    local char = lp.Character
    local hrp = char and char:FindFirstChild("HumanoidRootPart")
    if not hrp then return end
...
getgenv().SafeController = SafeExecutor
getgenv().SafeController:Execute(function()
    print("nil")
end)


-- Toggle Farm Level cực đơn giản
local FarmLv = Tabs.Main:AddToggle("ToggleFarmLv", {
    Title = "Cày Cấp",
    Description = "",
    Default = false
})
